L.Control.SlideMenu = L.Control.extend({
    ...
    onAdd: function(map){
        ...
        this._animateShow = (fn) => this._animate(this._menu, frominit, 0, true, ispx, unit, fn)
        this._animateHide = (fn) => this._animate(this._menu, 0, frominit, false, ispx, unit, fn);
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit, callback){
        if(this._isHorizontal){
            if(this._isLeftPosition){
        ...
        if(from != to){
            setTimeout(function(slideMenu){
                ...
                slideMenu._animate(slideMenu._menu, value, to, isOpen, ispx, unit, callback);
    ...
    show: function () {
        if (this._menu.style.display == 'inherit') {
            return;
        }
        this._menu.style.display = 'inherit';
        this._animateShow();
