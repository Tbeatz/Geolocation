<!-- Main modal -->
<div id="director-modal" aria-hidden="true"
    ...
    <div class="relative top-20 mx-auto p-5 border w-1/2 shadow-lg rounded-md bg-white">
        ...
        <div class="flex items-start justify-between p-4 border-b rounded-t border-gray-200">
            ...
            <button type="button" id="director_close"
        ...
        <!-- Modal body -->
        <form enctype="multipart/form-data" class="director_form" id="director_form_id">
            <div class="mt-2" id="director_message">
            </div>
            <div class="mt-2" id="director_error">
            </div>
            <div class="m-auto w-3/4">
                <div class="flex flex-wrap -mx-2 mt-2">
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input name="name" type="text" id="director_name" class="w-full" :value="old('name', $profile?->name)" required autofocus autocomplete="name" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="passport_nrc" :value="__('Passport/NRC')" />
                        <x-text-input name="passport_nrc" type="text" id="director_passport_nrc" class="w-full" :value="old('passport_nrc', $profile?->passport_nrc)" required autofocus autocomplete="passport_nrc" />
                        <x-input-error class="mt-2" :messages="$errors->get('passport_nrc')" />
                    </div>
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="address" :value="__('Address')" />
                        <x-text-input name="address" type="text" id="director_address" class="w-full" :value="old('address', $profile?->address)" required autofocus autocomplete="address" />
                        <x-input-error class="mt-2" :messages="$errors->get('address')" />
                    </div>
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="phone" :value="__('Phone')" />
                        <x-text-input name="phone" type="text" id="director_phone" class="w-full" :value="old('phone', $profile?->phone)" required autofocus autocomplete="phone" />
                        <x-input-error class="mt-2" :messages="$errors->get('phone')" />
                    </div>
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="email" :value="__('Email')" />
                        <x-text-input name="email" type="text" id="director_email" class="w-full" :value="old('email', $profile?->email)" required autofocus autocomplete="email" />
                        <x-input-error class="mt-2" :messages="$errors->get('email')" />
                    </div>
                    <div class="w-full md:w-1/2 px-2 mt-2">
                        <x-input-label for="nationality_id" :value="__('Nationality')" />
                        <select name="nationality_id" id="director_nationality_id" class="mt-1 p-2 text-sm block w-full font-arial border-gray-300 bg-white text-gray-700 focus:border-greenprimary focus:ring-lime-600 rounded-md shadow-sm" required>
